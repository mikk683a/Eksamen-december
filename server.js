...
//Opdater varer
app.get("/updategoods", checkAuthenticated, (req, res) => {
  res.render("updategoods.ejs")
})
app.put("/updategoods", async (req, res) => {
  try {
      
      // pusher opdateret information om varer til arrayet "goods"
      goods.push({
        pris: req.body.pris,
        kategori: req.body.kategori,
        beskrivelse: req.body.beskrivelse,
     ...
     // sletter eksisterende oplysninger ved hjælp af splice 
      users.splice(0, 1);  
      res.redirect("/") 
      // henter opdateret information fra arrayet "goods" og skriver det til goods.json
      fs.writeFileSync('database/goods.json', JSON.stringify(goods)); 
      console.log(users) 
  } catch {
      res.redirect("/updategoods") 
  }
})

//kategori menu
app.get("/kategori/:kategori", checkAuthenticated, (req,res) => {
  const kategorier = goods.find(k => k.kategori === req.params.kategori) 
